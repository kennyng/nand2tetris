...
Effects the actual compilation output. Gets parsed input from SyntaxAnalyzer and compiles VM code using VMWriter.
The output is generated by a series of compile_xxx() routines, one for every syntactic element xxx of the Jack grammar.
EXPORTED CLASSES: CompilationEngine
...
class CompilationEngine:
    ...
    def compile_SubroutineDec(self, subroutine, table):
        ...
        return getattr(self, "compile_" + subroutine_type.capitalize() + "Dec")(subroutine, new_table)
    ...
    def compile_FunctionDec(self, subroutine, table):
        ...
        output = []
        output.extend(VMWriter.write_function("%s.%s" %(self._class_name.identifier, subroutine.name.identifier), self._count_locals(subroutine)))
    ...
    def compile_ConstructorDec(self, subroutine, table):
        ...
        output = []
        output.extend(VMWriter.write_function("%s.%s" %(self._class_name.identifier, subroutine.name.identifier), self._count_locals(subroutine) + 1))
    ...
    def compile_Statement(self, statement, table):
        ...
        return getattr(self, "compile_" + name)(statement.statement, table)
    ...
    def compile_LetStatement(self, statement, table):
        ...
        return getattr(self, "compile_" + name)(statement.let_statement, table)

    def compile_RegularLetStatement(self, statement, table):
        value = table.find(statement.var_name.identifier)
        if value is None:
            raise ComplierError("UNKNOWN IDENTIFIER: %s" %statement.var_name.identifier)
        if value[1] == "field":
    ...
    def compile_ArrayEntryLetStatement(self, statement, table):
        value = table.find(statement.var_name.identifier)
        if value is None:
            raise CompilerError("UNKNOWN IDENTIFIER: %s" %statement.var_name.identifier)
        if value[1] == "field":
            value = (value[0], "this", value[2])
        ...
        output.extend(VMWriter.write_arithmetic("add"))
    ...
    def compile_IfStatement(self, statement, table):
        ...
        return getattr(self, "compile_" + name)(statement.if_statement, table)

    def compile_RegularIfStatement(self, statement, table):
        self._if_label_count += 1
        label = "END_IF_%d" %self._if_label_count

        output = []
        output.extend(self.compile_Expression(statement.expression, table))
        output.extend(VMWriter.write_arithmetic("not"))
    ...
    def compile_IfElseStatement(self, statement, table):
        self._if_label_count += 1
        label1 = "ELSE_CLAUSE_%d" %self._if_label_count
        label2 = "END_IF_%d" %self._if_label_count
    ...
    def compile_WhileStatement(self, statement, table):
        ...
        label1 = "START_WHILE_%d" %self._while_label_count
        label2 = "END_WHILE_%d" %self._while_label_count
    ...
    def compile_ReturnStatement(self, statement, table):
        ...
        return getattr(self, "compile_" + name)(statement.return_statement, table)
    ...
    def compile_Term(self, term, table):
        ...
        if name == "tuple":
            ...
            return getattr(self, "compile_" + name)(term.term, table)
    ...
    def compile_KeywordConstant(self, keyword_constant, table):
        ...
        if value == "true":
            ...
            output.extend(VMWriter.write_arithmetic("neg"))
            return output
        ...
        raise CompilerError("UNKNOWN KEYWORD CONSTANT: " + value)

    def compile_Identifier(self, identifier, table):
        """Compiles an identifier."""
        value = table.find(identifier.identifier)
        if value is None:
            raise CompilerError("UNKNOWN IDENTIFIER: " + identifier.identifier)
        if value[1] == "field":
            value = (value[0], "this", value[2])
    ...
    def compile_SubroutineCall(self, call, table):
        ...
        return getattr(self, "compile_" +  name)(call.subroutine_call, table)
    ...
    def compile_FunctionCall(self, call, table):
        ...
        output.extend(VMWriter.write_call("%s.%s" %(self._class_name.identifier, call.function_name.identifier), len(call.expression_list) + 1))
        return output
    ...
    def compile_MethodCall(self, call, table):
        ...
        output.extend(VMWriter.write_call("%s.%s" %(value[0], call.method_name.identifier), len(call.expression_list) + 1))
        return output
    ...
    def compile_StaticMethodCall(self, call, table):
        ...
        output.extend(VMWriter.write_call("%s.%s" %(class_name, call.method_name.identifier), len(call.expression_list)))
        return output
    ...
    def compile_UnaryOp(self, op, table):
        if op.op.symbol == "-":
            return VMWriter.write_arithmetic("neg")
        elif op.op.symbol == "~":
            return VMWriter.write_arithmetic("not")
        else:
            raise CompilerError("'%s' IS NOT A UNARY OPERATOR" %op.op.symbol)
