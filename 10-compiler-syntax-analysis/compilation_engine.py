...
into an output file/stream. The output is generated by a series of compilexx() routines, one for every
...
class CompilationEngine:
    ...
    def __init__(self, tokenizer, output_file):
        """Creates a new compilation engine with the given inpit and output."""
    ...
    def compile_class(self):
        ...
        if self._tokenizer.keyword() != 'class': 
            raise SyntacticError("In <class>: expected 'class' but got '%s'" %self._tokenizer.token())
        self._write_token_and_advance(_indent + 1)

        # Check for class name
        if not self._tokenizer.identifier():
            raise SyntacticError("In <class>: expected [class identifier] but got '%s'" %self._tokenizer.token())
        self._write_token_and_advance(_indent + 1)

        if self._tokenizer.symbol() != '{': 
            raise SyntacticError("In <class>: expected '{' but got '%s'" %self._tokenizer.token())
        self._write_token_and_advance(_indent + 1)
    ...
    def compile_class_var_dec(self):
        ...
        if self._tokenizer.keyword() not in ("static", "field"):
            raise SyntacticError("In <classVarDec>: expected 'static' | 'field' but got '%s'" %self._tokenizer.token())
        self._write_token_and_advance(_indent + 1)
        ...
        if self._tokenizer.keyword() not in ('int', 'char', 'boolean') and not self._tokenizer.identifier():
            raise SyntacticError("In <classVarDec>: expected 'int' | 'char' | 'boolean' but got '%s'" %self._tokenizer.token())
        self._write_token_and_advance(_indent + 1)

        # Check for variable name
        if not self._tokenizer.identifier():
            raise SyntacticError("In <classVarDec>: expected [var identifier] but got '%s'" %self._tokenizer.token())
        self._write_token_and_advance(_indent + 1)
        ...
        if not self._tokenizer.symbol() == ';':
            raise SyntacticError("In <classVarDec>: expected ';' but got '%s'" %self._tokenizer.token())
        self._write_token_and_advance(_indent + 1)
        self._write("</classVarDec>", _indent)
    ...
    def compile_subroutine(self):
        ...
        if self._tokenizer.keyword() not in ('constructor', 'function', 'method'):
            raise SyntacticError("In <subroutineDec>: expected 'constructor' | 'function' | 'method' but got '%s'" %self._tokenizer.token())
        self._write_token_and_advance(_indent + 1)
        ...
        if self._tokenizer.keyword() not in ('void', 'int', 'char', 'boolean') and not self._tokenizer.identifier():
            raise SyntacticError("In <subroutineDec>: expected 'void' | 'int' | 'char' | 'boolean' | [identifier] but got '%s'" %self._tokenizer.token())      
        self._write_token_and_advance(_indent + 1)

        # Check for function name
        if not self._tokenizer.identifier():
            raise SyntacticError("In <subroutineDec>: expected [function identifier] but got '%s'" %self._tokenizer.token())
        self._write_token_and_advance(_indent + 1)

        if self._tokenizer.symbol() != '(':
            raise SyntacticError("In <subroutineDec>: expected '(' but got '%s'" %self._tokenizer.token())
        self._write_token_and_advance(_indent + 1)
        ...
        if self._tokenizer.symbol() != '{':
            raise SyntacticError("In <subroutineBody>: expected '{' but got '%s'" %self._tokenizer.token())
    ...
    def compile_parameter_list(self):
        ...
        # Check var name
        if not self._tokenizer.identifier():
            raise SyntacticError("In <parameterList>: expected [var identifier] but got '%s'" %self._tokenizer.token())
        self._write_token_and_advance(_indent)
        ...
        while self._tokenizer.symbol() == ',':
            ...
            if self._tokenizer.keyword() not in ('int', 'char', 'boolean') and not self._tokenizer.identifier():
                raise SyntacticError("In <parameterList>: expected 'int' | 'char' | 'boolean' | [identifier] but got '%s'" %self._tokenizer.token())
            self._write_token_and_advance(_indent)
            # Check var name
            if not self._tokenizer.identifier():
                raise SyntacticError("In <parameterList>: expected [var identifier] but got '%s'" %self._tokenizer.token())
    ...
    def compile_var_dec(self):
        ...
        if self._tokenizer.keyword() != 'var':
            raise SyntacticError("In <varDec>: expected 'var' but got '%s'" %self._tokenizer.token())
        self._write_token_and_advance(_indent + 1)
        
        # Check var type or identifier
        if self._tokenizer.keyword() not in ('int', 'char', 'boolean') and not self._tokenizer.identifier():
            raise SyntacticError("In <varDec>: expected 'int' | 'char' | 'boolean' | [var identifier] but got '%s'" %self._tokenizer.token())
        self._write_token_and_advance(_indent + 1)

        # Check var name
        if not self._tokenizer.identifier():
            raise SyntacticError("In <varDec>: expected [var identifier] but got '%s'" %self._tokenizer.token())
        self._write_token_and_advance(_indent + 1)
        ...
        if self._tokenizer.symbol() != ';':
            raise SyntacticError("In <varDec>: expected ';' but got '%s'" %self._tokenizer.token())
        self._write_token_and_advance(_indent + 1)
        self._write("</varDec>", _indent)
    ...
    def compile_do(self):
        ...
        if self._tokenizer.keyword() != 'do':
            raise SyntacticError("In <doStatement>: expected 'do' but got '%s'" %self._tokenizer.token())
        self._write_token_and_advance(_indent + 1)
        ...
        if self._tokenizer.symbol() != ';':
            raise SyntacticError("In <doStatement>: expected ';' but got '%s'" %self._tokenizer.token())
        self._write_token_and_advance(_indent + 1)
    ...
    def _compile_subroutine_call(self):
        ...
        if not self._tokenizer.identifier():
            raise SyntacticError("In SubroutineCall: expected [identifier] but got '%s'" %self._tokenizer.token())
        self._write_token_and_advance(_indent)
        ...
        if self._tokenizer.symbol() == '(':
            ...
            if self._tokenizer.symbol() != ')':
                raise SyntacticError("In SubroutineCall: expected ')' but got '%s'" %self._tokenizer.token())
        ...
        # Check for method call
            ...
            if not self._tokenizer.identifier():
                raise SyntacticError("In SubroutineCall: expected [method identifier] but got '%s'" %self._tokenizer.token())
            self._write_token_and_advance(_indent)
            if self._tokenizer.symbol() != '(':
                raise SyntacticError("In SubroutineCall: expected '(' but got '%s'" %self._tokenizer.token())
            self._write_token_and_advance(_indent)
            ...
            raise SyntacticError("In SubroutineCall: expected '(' or '.' but got '%s'" %self._tokenizer.token())
    ...
    def compile_let(self):
        ...
        if self._tokenizer.keyword() != 'let':
            raise SyntacticError("In <letStatement>: expected 'let' but got '%s'" %self._tokenizer.token())
        self._write_token_and_advance(_indent + 1)
        
        # Check for identifier
        if not self._tokenizer.identifier():
            raise SyntacticError("In <letStatement>: expected [identifier] but got '%s'" %self._tokenizer.token())
        self._write_token_and_advance(_indent + 1)
        ...
        if self._tokenizer.symbol() == '[':
            ...
            if self._tokenizer.symbol() != ']':
                raise SyntacticError("In <letStatement>: expected ']' but got '%s'" %self._tokenizer.token())
            self._write_token_and_advance(_indent + 1)
    ...
    def compile_while(self):
        ...
        if self._tokenizer.keyword() != 'while':
            raise SyntacticError("In <whileStatement>: expected 'while' but got '%s'" %self._tokenizer.token())
        self._write_token_and_advance(_indent + 1)
        
        if self._tokenizer.symbol() != '(':
            raise SyntacticError("In <whileStatement>: expected '(' but got '%s'" %self._tokenizer.token())
        self._write_token_and_advance(_indent + 1)
        self._rel_indent = _indent + 1
        self.compile_expression()
        if self._tokenizer.symbol() != ')':
    ...
    def compile_return(self):
        ...
        if self._tokenizer.keyword() != 'return':
            raise SyntacticError("In <returnStatement>: expected 'return' but got '%s'" %self._tokenizer.token())
        self._write_token_and_advance(_indent + 1)
        ...
        if self._tokenizer.symbol() != ';':
            raise SyntacticError("In <returnStatement>: expected ';' but got '%s'" %self._tokenizer.token())
        self._write_token_and_advance(_indent + 1)
    ...
    def compile_if(self):
        ...
        if self._tokenizer.keyword() != 'if':
            raise SyntacticError("In <ifStatement>: expected 'if' but got '%s'" %self._tokenizer.token())
        self._write_token_and_advance(_indent + 1)
        
        if self._tokenizer.symbol() != '(':
            raise SyntacticError("In <ifStatement>: expected '(' but got '%s'" %self._tokenizer.token())
        self._write_token_and_advance(_indent + 1)
        self._rel_indent = _indent + 1
        self.compile_expression()
        if self._tokenizer.symbol() != ')':
    ...
    def compile_term(self):
        ...
        # Check for keyword constant
            ...
            if self._tokenizer.keyword() not in ('true', 'false', 'null', 'this'):
                raise SyntacticError("In <term>: expected 'true' | 'false' | 'null' | 'this' but got '%s'" %self._tokenizer.token())
            _is_term = True
            self._write_token(self._rel_indent)
        # Check for expression
        elif self._tokenizer.symbol() == '(':
            ...
            self.compile_expression()
            if self._tokenizer.symbol() != ')':
                raise SyntacticError("In <term>: expected ')' but got '%s'" %self._tokenizer.token())
            _is_term = True
    ...
    def _lookahead_check(self):
        ...
        # Check for variable -> subroutine call
            ...
            # Check for identifier
            if not self._tokenizer.identifier():
                raise SyntacticError("In <term>: expected [identifier] but got '%s'" %self._tokenizer.token())
            self._write_token_and_advance(_lc_indent)
            if self._tokenizer.symbol() != '(':
                raise SyntacticError("In <term>: expected '(' but got '%s'" %self._tokenizer.token())
            self._write_token_and_advance(_lc_indent)
...
class SyntacticError(Exception):
    """For syntax error exceptions; null operation -- nothing happens."""
    pass
